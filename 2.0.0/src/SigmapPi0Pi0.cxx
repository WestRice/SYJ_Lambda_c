...
bool has_identical_gam(std::vector<int> &all_gamma)
{
	for (int i = 0; i < all_gamma.size(); ++i) 
		for (int j = i + 1;  j < all_gamma.size(); j++) 
	{
		if( all_gamma[i] == all_gamma[j])
			return true;
	}
...
double find_min_ang(std::vector<Gam_info> &iGam, std::vector<int> &all_gamma)
{
	double min = 99999;

	for (int i = 0; i < all_gamma.size();  ++i) 
	{
		if(iGam[all_gamma[i]].ang_antiproton < min)
		{
			min = iGam[all_gamma[i]].ang_antiproton;
		}
	}
	return min;
}

bool SigmapPi0Pi0::isGoodSigmap(RecMdcKalTrack *ppTrk, RecEmcShower *shr1, RecEmcShower *shr2)
{
	...
	HepLorentzVector sigmap_p4 = ppTrk->p4(PDG::Proton) + getP4(shr1,xorigin) + getP4(shr2,xorigin);

	if( sigmap_p4.m() > 1.176 && sigmap_p4.m() < 1.20)
	{
		return true;
	}
	else
...
SigmapPi0Pi0::SigmapPi0Pi0(const std::string& name, ISvcLocator* pSvcLocator):Algorithm(name,pSvcLocator){
...
void SigmapPi0Pi0::addItem()
{
	StatusCode status;
        ...
        status = m_tuple->addIndexedItem ("gam0_p4",m_index, 4, m_gam0_p4);
        status = m_tuple->addIndexedItem ("gam1_p4",m_index, 4, m_gam1_p4);
        status = m_tuple->addIndexedItem ("gam2_p4",m_index, 4, m_gam2_p4);
        status = m_tuple->addIndexedItem ("gam3_p4",m_index, 4, m_gam3_p4);
        status = m_tuple->addIndexedItem ("gam4_p4",m_index, 4, m_gam4_p4);
        status = m_tuple->addIndexedItem ("gam5_p4",m_index, 4, m_gam5_p4);

        status = m_tuple->addIndexedItem ("gam0_p4_before",m_index, 4, m_gam0_p4_before);
        status = m_tuple->addIndexedItem ("gam1_p4_before",m_index, 4, m_gam1_p4_before);
        status = m_tuple->addIndexedItem ("gam2_p4_before",m_index, 4, m_gam2_p4_before);
        status = m_tuple->addIndexedItem ("gam3_p4_before",m_index, 4, m_gam3_p4_before);
        status = m_tuple->addIndexedItem ("gam4_p4_before",m_index, 4, m_gam4_p4_before);
        status = m_tuple->addIndexedItem ("gam5_p4_before",m_index, 4, m_gam5_p4_before);

        status = m_tuple->addIndexedItem ("ang_gam_antiproton",m_index, m_ang_gam_antiproton);

        status = m_tuple->addIndexedItem ("lmdc_chis",m_index, m_lmdc_chis);
...
StatusCode SigmapPi0Pi0::initialize(){
		MsgStream log(msgSvc(), name());
		log<<MSG::INFO<<"SigmapPi0Pi0::initialize()"<<endreq;
...
StatusCode SigmapPi0Pi0::beginRun(){
	if(m_debug) std::cerr<<"!!!!!!!!!!!!!!!!!"<<"Enter beginRun"<<"!!!!!!!!!!"<<std::endl;
	MsgStream log(msgSvc(), name());
	log<<MSG::INFO<<"SigmapPi0Pi0::beginRun()"<<endreq;
	//add your code here
	return StatusCode::SUCCESS;

}
StatusCode SigmapPi0Pi0::execute(){

	if(m_debug) std::cerr<<"!!!!!!!!!!!!!!!!!"<<"Enter execute"<<"!!!!!!!!!!"<<std::endl;
	MsgStream log(msgSvc(), name());
	log<<MSG::INFO<<"SigmapPi0Pi0::execute()"<<endreq;
	...
	if (eventHeader->runNumber()<0 && m_checktotal)
	{
		...
		else
		{
			...
			for(; iter_mc != mcParticleCol->end(); ++iter_mc)
			{
				...
				//mode2
				if( /*&& mode2 == 11*/  pdg == 3222 && motherpdg == 4122) //sigma0 -> lmd gam
				{
					const SmartRefVector<Event::McParticle>& gc = (*iter_mc)->daughterList();
					for(unsigned int ii = 0; ii < gc.size(); ii++) 
					{
				...
				if( /*&& mode2 == 11*/  pdg == 111 && (motherpdg == 4122 || (motherpdg == 3222 && mmotherpdg == 4122))) //pi0 -> gam gam
				{
					const SmartRefVector<Event::McParticle>& gc = (*iter_mc)->daughterList();
					for(unsigned int ii = 0; ii < gc.size(); ii++) 
					{
				...
				//mode2
				if(/*&& mode3 == 11*/  pdg == -3222 && motherpdg == -4122) //sigma0 -> lmd gam
				{
					const SmartRefVector<Event::McParticle>& gc = (*iter_mc)->daughterList();
					for(unsigned int ii = 0; ii < gc.size(); ii++) 
					{
	...
	if(m_debug) std::cerr<<"!!!!!!!!!!!!!!!!!"<<"Enter Good Sigma+"<<"!!!!!!!!!!"<<std::endl;
	
	Vint iSigmap_proton;
	Vint iSigmap_pi0;
	for (int ip = 0;  ip < iProtonp_loose.size(); ip++) 
	{
		for (int ig = 0; ig < iPi0gam1.size(); ++ig) 
		{
			EvtRecTrackIterator itTrk_1 = evtRecTrkCol->begin() + iProtonp_loose[ip];
			RecMdcKalTrack *ppKalTrk = (*itTrk_1)->mdcKalTrack();

			EvtRecTrackIterator itTrk_2 = evtRecTrkCol->begin() + iGam[iPi0gam1[ig]].index;
			RecEmcShower* shr1 = (*itTrk_2)->emcShower();

			EvtRecTrackIterator itTrk_3 = evtRecTrkCol->begin() + iGam[iPi0gam2[ig]].index;
			RecEmcShower* shr2 = (*itTrk_3)->emcShower();

			if(isGoodSigmap(ppKalTrk, shr1, shr2))
			{
				iSigmap_proton.push_back(ip);
				iSigmap_pi0.push_back(ig);
			}
	...
	Vint iAnti_Sigmam_proton;
	Vint iAnti_Sigmam_pi0;
	for (int ip = 0;  ip < iProtonm_loose.size(); ip++) 
	{
		for (int ig = 0; ig < iPi0gam1.size(); ++ig) 
		{
			EvtRecTrackIterator itTrk_1 = evtRecTrkCol->begin() + iProtonm_loose[ip];
			RecMdcKalTrack *pmKalTrk = (*itTrk_1)->mdcKalTrack();
			...
			if(isGoodSigmap(pmKalTrk, shr1, shr2))
			{
				iAnti_Sigmam_proton.push_back(ip);
				iAnti_Sigmam_pi0.push_back(ig);
			}
	...
	for(int i_sigmap = 0; i_sigmap < iSigmap_proton.size(); i_sigmap++)
	{
		for(int i_pi01=0; i_pi01< iPi0gam1.size(); i_pi01++)
		{

			for(int i_pi02 = i_pi01 + 1; i_pi02 < iPi0gam1.size(); i_pi02++)
			{
				if(m_debug) cerr<<"Enter Loop!!!!!!!!!"<<endl;
				std::vector<int> igam;
				std::vector<RecEmcShower*> showers;

				igam.push_back(iPi0gam1[i_pi01]);
				EvtRecTrackIterator it_temp = evtRecTrkCol->begin() + iGam[iPi0gam1[i_pi01]].index;
				showers.push_back((*it_temp)->emcShower());

				igam.push_back(iPi0gam2[i_pi01]);
				it_temp = evtRecTrkCol->begin() + iGam[iPi0gam2[i_pi01]].index;
				showers.push_back((*it_temp)->emcShower());

				igam.push_back(iPi0gam1[i_pi02]);
				it_temp = evtRecTrkCol->begin() + iGam[iPi0gam1[i_pi02]].index;
				showers.push_back((*it_temp)->emcShower());

				igam.push_back(iPi0gam2[i_pi02]);
				it_temp = evtRecTrkCol->begin() + iGam[iPi0gam2[i_pi02]].index;
				showers.push_back((*it_temp)->emcShower());


				if(m_debug) cerr<<"Enter all gamma!!!!!!!!!"<<endl;
				igam.push_back(iPi0gam1[iSigmap_pi0[i_sigmap]]);
				it_temp = evtRecTrkCol->begin() + iGam[iPi0gam1[iSigmap_pi0[i_sigmap]]].index;
				showers.push_back((*it_temp)->emcShower());

				igam.push_back(iPi0gam2[iSigmap_pi0[i_sigmap]]);
				it_temp = evtRecTrkCol->begin() + iGam[iPi0gam2[iSigmap_pi0[i_sigmap]]].index;
				showers.push_back((*it_temp)->emcShower());

				if(m_debug) cerr<<"End all gamma!!!!!!!!!"<<endl;
				if(has_identical_gam(igam))
					continue;

				double ang_gam_antiproton = find_min_ang(iGam,igam);

				EvtRecTrackIterator itTrk_p = evtRecTrkCol->begin() 
				+ iProtonp_loose[iSigmap_proton[i_sigmap]];
				RecMdcKalTrack *ppKalTrk = (*itTrk_p)->mdcKalTrack();
				RecMdcKalTrack::setPidType(RecMdcKalTrack::proton);
				WTrackParameter wtp_proton(xmass[4], ppKalTrk->getZHelixP(),ppKalTrk->getZErrorP());
				...
				kmfit->AddTrack(0, 0., showers[0]);
				kmfit->AddTrack(1, 0., showers[1]);
				kmfit->AddTrack(2, 0., showers[2]);
				kmfit->AddTrack(3, 0., showers[3]);
				kmfit->AddTrack(4, 0., showers[4]);
				kmfit->AddTrack(5, 0., showers[5]);
				kmfit->AddTrack(6, wtp_proton);

				kmfit->AddResonance(0, PDG::Sigmap, 4, 5, 6);
				kmfit->AddResonance(1, PDG::Pi0, 0, 1);
				kmfit->AddResonance(2, PDG::Pi0, 2, 3);
				kmfit->AddResonance(3, PDG::Pi0, 4, 5);

				bool okvs=kmfit->Fit();
				if(!okvs) continue;
				//boost
				double lmdc_chisq = kmfit -> chisq();
				double mbc = -10;
				HepLorentzVector tot_p4, tot_lmd;
				//if(!Mass1c_Lmdc(wtp_sigma0, wtp_pip, wtp_pi0,lmdc_chisq, tot_p4))
				//	continue;

				tot_p4 = kmfit -> pfit(0) + kmfit -> pfit(1) + kmfit -> pfit(2)
				...
				else 
				{
					...
					{
						m_gam0_p4[index][pp] = kmfit->pfit(0)[pp];
						m_gam1_p4[index][pp] = kmfit->pfit(1)[pp];
						m_gam2_p4[index][pp] = kmfit->pfit(2)[pp];
						m_gam3_p4[index][pp] = kmfit->pfit(3)[pp];
						m_gam4_p4[index][pp] = kmfit->pfit(4)[pp];
						m_gam5_p4[index][pp] = kmfit->pfit(5)[pp];

						m_gam0_p4_before[index][pp] = p4Pi0gam1[i_pi01][pp];
						m_gam1_p4_before[index][pp] = p4Pi0gam2[i_pi01][pp];
						m_gam2_p4_before[index][pp] = p4Pi0gam1[i_pi02][pp];
						m_gam3_p4_before[index][pp] = p4Pi0gam2[i_pi02][pp];
						m_gam4_p4_before[index][pp] = p4Pi0gam1[iSigmap_pi0[i_sigmap]][pp];
						m_gam5_p4_before[index][pp] = p4Pi0gam2[iSigmap_pi0[i_sigmap]][pp];
					}
	...
	for(int i_anti_sigmam = 0; i_anti_sigmam < iAnti_Sigmam_proton.size(); i_anti_sigmam++)
	{
		for(int i_pi01=0; i_pi01< iPi0gam1.size(); i_pi01++)
		{
			...
			for(int i_pi02 = i_pi01 + 1; i_pi02 < iPi0gam1.size(); i_pi02++)
			{
				...
				igam.push_back(iPi0gam1[iAnti_Sigmam_pi0[i_anti_sigmam]]);
				it_temp = evtRecTrkCol->begin() + iGam[iPi0gam1[iAnti_Sigmam_pi0[i_anti_sigmam]]].index;
				showers.push_back((*it_temp)->emcShower());

				igam.push_back(iPi0gam2[iAnti_Sigmam_pi0[i_anti_sigmam]]);
				it_temp = evtRecTrkCol->begin() + iGam[iPi0gam2[iAnti_Sigmam_pi0[i_anti_sigmam]]].index;
				showers.push_back((*it_temp)->emcShower());
				...
				EvtRecTrackIterator itTrk_m = evtRecTrkCol->begin() 
				+ iProtonm_loose[iAnti_Sigmam_proton[i_anti_sigmam]];
				RecMdcKalTrack *pmKalTrk = (*itTrk_m)->mdcKalTrack();
				RecMdcKalTrack::setPidType(RecMdcKalTrack::proton);
				WTrackParameter wtp_proton(xmass[4], pmKalTrk->getZHelixP(),pmKalTrk->getZErrorP());
				...
				else 
				{
					...
					for(int pp=0;pp<4;pp++)
					{
						...
						m_gam2_p4_before[index][pp] = p4Pi0gam1[i_pi02][pp];
						m_gam3_p4_before[index][pp] = p4Pi0gam2[i_pi02][pp];
						m_gam4_p4_before[index][pp] = p4Pi0gam1[iAnti_Sigmam_pi0[i_anti_sigmam]][pp];
						m_gam5_p4_before[index][pp] = p4Pi0gam2[iAnti_Sigmam_pi0[i_anti_sigmam]][pp];
					}
...
StatusCode SigmapPi0Pi0::endRun(){
	MsgStream log(msgSvc(), name());
	log<<MSG::INFO<<"SigmapPi0Pi0::endRun()"<<endreq;
	//add your code here
	if(m_debug) std::cerr<<"!!!!!!!!!!!!!!!!!"<<"Enter endRun"<<"!!!!!!!!!!"<<std::endl;
	return StatusCode::SUCCESS;

}
StatusCode SigmapPi0Pi0::finalize(){
	MsgStream log(msgSvc(), name());
	log<<MSG::INFO<<"SigmapPi0Pi0::finalize()"<<endreq;
...
bool SigmapPi0Pi0::isGoodTrk(EvtRecTrackIterator itTrk, double &vz , double &vxy) {
...
bool SigmapPi0Pi0::isProton(EvtRecTrackIterator itTrk)
{
...
bool SigmapPi0Pi0::isPion( EvtRecTrackIterator itTrk )
{
		//double m_prob_cut =0.001;
		if(!(*itTrk)->isMdcKalTrackValid()) return false;
		ParticleID *pid = ParticleID::instance();
...
bool SigmapPi0Pi0::isKaon( EvtRecTrackIterator itTrk )
{
		//double m_prob_cut =0.001;
		if(!(*itTrk)->isMdcKalTrackValid()) return false;
		ParticleID *pid = ParticleID::instance();
...
bool SigmapPi0Pi0::isGoodShower(EvtRecTrack* trk, double& ang_gam_antiproton){
	...
	//if(fabs(dang) < 0. ) return false;

	return true;
}

bool SigmapPi0Pi0::isGoodLambda(RecMdcKalTrack* ppTrk, RecMdcKalTrack* pimTrk, double& lmd_1chis, double& lmd_2chis, double& lmd_lchue, WTrackParameter &swvlmd, double& lmd_mass){
		lmd_1chis=-100;
		lmd_2chis=-100;
		lmd_lchue =-100;
		lmd_mass =-100;
...
bool SigmapPi0Pi0::isGoodPi0(RecEmcShower *shr1,RecEmcShower *shr2,double& pi0_mass,HepLorentzVector& p4_pi0,double& pi0_chis,HepLorentzVector& p4_pi0_1c,
...
bool SigmapPi0Pi0::isGoodSigma0(WTrackParameter *lmd,RecEmcShower *gammaShr,double& sigma0_mass,HepLorentzVector& p4_sigma0,double& sigma0_chis,HepLorentzVector& p4_sigma0_1c, HepLorentzVector &p4_gam, WTrackParameter &wtp_sigma0_1c){
...
bool SigmapPi0Pi0::Mass1c_Lmdc(WTrackParameter &sigma0, WTrackParameter &piTrk, WTrackParameter &pi0Trk,double& lmdc_chis,HepLorentzVector& p4_lmdc_1c){
		lmdc_chis=-100;
...
HepLorentzVector SigmapPi0Pi0::getP4(RecEmcShower* gTrk, Hep3Vector origin){
...
bool SigmapPi0Pi0::isGoodTrackForLambda(EvtRecTrack* trk){
