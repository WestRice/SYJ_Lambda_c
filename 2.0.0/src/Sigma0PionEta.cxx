...
void Sigma0PionEta::addItem(int idx)
{
	StatusCode status;
 ...
 //       status = m_tuple[idx]->addIndexedItem ("Rz",m_index[idx], m_z[idx]);
        ...
        status = m_tuple[idx]->addItem ("eta_gam1_p4",m_index[idx], m_eta_gam1_p4[idx]);
        status = m_tuple[idx]->addItem ("eta_gam2_p4",m_index[idx], m_eta_gam2_p4[idx]);
...
StatusCode Sigma0PionEta::initialize(){
		...
		//add your code here
		StatusCode status;
		NTuplePtr nt1(ntupleSvc(), "FILE2/Lp");
		if ( nt1 ) m_tuple[0] = nt1;
		else {
				m_tuple[0] = ntupleSvc()->book ("FILE2/Lp", CLID_ColumnWiseTuple, "exam N-Tuple example");
				if ( m_tuple[0] )    
				{
				addItem(0);
				}
		...
		NTuplePtr nt2(ntupleSvc(), "FILE2/Lm");
		if ( nt2 ) m_tuple[1] = nt2;
		else {
				m_tuple[1] = ntupleSvc()->book ("FILE2/Lm", CLID_ColumnWiseTuple, "exam N-Tuple example");
				if ( m_tuple[1] )    
				{
				addItem(1);
				}
		...
		if(m_checktotal)
		{
		NTuplePtr nt_mc(ntupleSvc(), "FILE2/mc_truth");
		if ( nt_mc ) m_tuple_mc = nt_mc;
		else {
				m_tuple_mc = ntupleSvc()->book ("FILE2/mc_truth", CLID_ColumnWiseTuple, "exam N-Tuple example");
				if ( m_tuple_mc )    
				{
...
StatusCode Sigma0PionEta::execute(){
	...
	if(m_debug) std::cerr<<"!!!!!!!!!!!!!!!!!"<<"Enter execute"<<"!!!!!!!!!!"<<std::endl;
		...
		//initi
		const int LAMBDAC_SIGMA0_PION_ETA = 11;
	...
	if (eventHeader->runNumber()<0 && m_checktotal)
	{
		...
		else
		{
			...
			for(; iter_mc != mcParticleCol->end(); ++iter_mc)
			{
				...
				//mode2
				//if(mode1 == 0 && mode2 == LAMBDAC_SIGMA0_PION_ETA  && pdg == 3212 && motherpdg == 4122) //sigma0 -> lmd gam
				...
				//if(mode1 == 0 && mode2 == LAMBDAC_SIGMA0_PION_ETA && pdg == 111 && motherpdg == 4122) //eta -> gam gam
				if(mode1 == 0 && mode2 == 0 && pdg == 221 && motherpdg == 4122) //eta -> gam gam
				{
					const SmartRefVector<Event::McParticle>& gc = (*iter_mc)->daughterList();
					for(unsigned int ii = 0; ii < gc.size(); ii++) 
					{
				...
				//if(mode1 == 0 && mode2 == LAMBDAC_SIGMA0_PION_ETA && pdg == 3122 && motherpdg == 3212 && mmotherpdg == 4122) //lmd -> p pi
				...
				//mode2
				//if(mode1 == 0 && mode3 == LAMBDAC_SIGMA0_PION_ETA && pdg == -3212 && motherpdg == -4122) //sigma0 -> lmd gam
				...
				//if(mode1 == 0 && mode3 == LAMBDAC_SIGMA0_PION_ETA && pdg == 111 && motherpdg == -4122) //eta -> gam gam
				if(mode1 == 1 && mode3 == 0 && pdg == 221 && motherpdg == -4122) //eta -> gam gam
				{
					const SmartRefVector<Event::McParticle>& gc = (*iter_mc)->daughterList();
					for(unsigned int ii = 0; ii < gc.size(); ii++) 
					{
				...
				//if(mode1 == 0 && mode3 == LAMBDAC_SIGMA0_PION_ETA  && pdg == -3122 && motherpdg == -3212 && mmotherpdg == -4122) //lmd -> p pi
	...
	if(m_debug) std::cerr<<"!!!!!!!!!!!!!!!!!"<<"Enter Good Eta"<<"!!!!!!!!!!"<<std::endl;
		for(int i = 0; i < iGam.size()-1; i++) 
		{
			for(int j = i+1; j < iGam.size(); j++) 
			{
				...
				if(isGoodEta(shr1,shr2,eta_mass,p4_eta,eta_chis,p4_eta_1c, p4_gam1, p4_gam2)){
	...
	for(int i_pionp=0;i_pionp<iPionp.size();i_pionp++)
	{
		...
		for(int i_sigma0 = 0; i_sigma0 < p4Sigma01c.size(); i_sigma0++)
		{
			...
			for(int i_eta = 0; i_eta < p4Eta1c.size(); i_eta++)
			{
				...
				HepLorentzVector p4_eta = p4Eta1c[i_eta];	
				//double check_rho = (p4_pionp + p4_eta).m();
				//if(Cut::Rho(check_rho)) continue;

				//boost 
				HepLorentzVector cms(0,0,0,m_ecms);
				HepLorentzVector tot_p4= p4_pionp + p4_sigma0 + p4_eta;
				...
				if(m_BestCandidate)
				{
					if(fabs(deltaE)<tmp_De_lp)
					{
						...
						for(int pp=0;pp<4;pp++)
						{
							...
							m_fourmom[0][2][pp] = p4_eta[pp];
							m_lambda_p4[0][pp]  = p4_lambda[pp]; 

							m_eta_gam1_p4[0][pp] = p4Etagam1[i_eta][pp];
							m_eta_gam2_p4[0][pp] = p4Etagam2[i_eta][pp];
	...
	for(int i_pionm = 0; i_pionm < iPionm.size(); i_pionm++)
	{
		...
		for(int i_sigma0 = 0; i_sigma0 < p4ASigma01c.size(); i_sigma0++)
		{
			...
			for(int i_eta = 0; i_eta < p4Eta1c.size(); i_eta++)
			{
				...
				HepLorentzVector p4_eta = p4Eta1c[i_eta];	
				//double check_rho = (p4_pionm + p4_eta).m();
				//if(Cut::Rho(check_rho)) continue;
				//boost 
				HepLorentzVector cms(0,0,0,m_ecms);
				HepLorentzVector tot_p4= p4_pionm + p4_sigma0 + p4_eta;
				HepLorentzVector tot_p4_boost;
				tot_p4_boost = tot_p4.boost(-0.011,0.,0.);
				double mbc2 = m_beamE*m_beamE- tot_p4_boost.v().mag2();
				double mbc = mbc2 > 0 ? sqrt( mbc2 ) : -10;
				...
				if(m_BestCandidate)
				{
					if(fabs(deltaE)<tmp_De_lm)
					{
						...
						for(int pp=0;pp<4;pp++)
						{
							...
							m_fourmom[1][2][pp] = p4_eta[pp];
							m_lambda_p4[1][pp]  = p4_lambda[pp]; 

							m_eta_gam1_p4[1][pp] = p4Etagam1[i_eta][pp];
							m_eta_gam2_p4[1][pp] = p4Etagam2[i_eta][pp];
...
bool Sigma0PionEta::isPronton(EvtRecTrackIterator itTrk)
{
...
bool Sigma0PionEta::isGoodSigma0(WTrackParameter *lmd,RecEmcShower *gammaShr,double& sigma0_mass,HepLorentzVector& p4_sigma0,double& sigma0_chis,HepLorentzVector& p4_sigma0_1c, HepLorentzVector &p4_gam){
