...
void Sigma0PipPipPim::addItem(int idx)
{
 ...
 //       status = m_tuple[idx]->addItem ("pi0_gam1_p4",m_index[idx], m_pi0_gam1_p4[idx]);
 //       status = m_tuple[idx]->addItem ("pi0_gam2_p4",m_index[idx], m_pi0_gam2_p4[idx]);
...
StatusCode Sigma0PipPipPim::execute(){
	...
	if(m_debug) std::cerr<<"!!!!!!!!!!!!!!!!!"<<"Enter execute"<<"!!!!!!!!!!"<<std::endl;
		...
		//initi
		const int LAMBDAC_SIGMA0_PIP_PIP_PIM = 11;
	...
	if (eventHeader->runNumber()<0 && m_checktotal)
	{
		...
		else
		{
			...
			for(; iter_mc != mcParticleCol->end(); ++iter_mc)
			{
				...
				//if(pdg == 4122 && mode1 == 0 && mode2 == LAMBDAC_SIGMA0_PIP_PIP_PIM) //lambda_c+
				if(pdg == 4122 && mode1 == 0 ) //lambda_c+
				{
					...
					m_flag[0] = LAMBDAC_SIGMA0_PIP_PIP_PIM;
				}

				//if (pdg == -4122 && mode1 == 0 && mode3 == LAMBDAC_SIGMA0_PIP_PIP_PIM) //lambda_c-
				if (pdg == -4122 && mode1 == 1 ) //lambda_c-
				{
					...
					m_flag[1] = LAMBDAC_SIGMA0_PIP_PIP_PIM;
				}

				//mode2
				//if(mode1 == 0 && mode2 == LAMBDAC_SIGMA0_PIP_PIP_PIM  && pdg == 3212 && motherpdg == 4122) //sigma0 -> lmd gam
				...
				//if(mode1 == 0 && mode2 == LAMBDAC_SIGMA0_PIP_PIP_PIM && pdg == 3122 && motherpdg == 3212 && mmotherpdg == 4122) //lmd -> p pi
				...
				//mode2
				//if(mode1 == 0 && mode3 == LAMBDAC_SIGMA0_PIP_PIP_PIM && pdg == -3212 && motherpdg == -4122) //sigma0 -> lmd gam
				...
				//if(mode1 == 0 && mode3 == LAMBDAC_SIGMA0_PIP_PIP_PIM  && pdg == -3122 && motherpdg == -3212 && mmotherpdg == -4122) //lmd -> p pi
	...
	for(int i_pionp2 = i_pionp1 + 1; i_pionp2 < iPionp.size(); i_pionp2++)
	{
		...
		RecMdcKalTrack::setPidType  (RecMdcKalTrack::pion);
		HepLorentzVector p4_pionp2 = ((*itTrk)->mdcKalTrack())->p4(PDG::Pion);
	...
	{
		EvtRecTrackIterator itTrk=evtRecTrkCol->begin() + iPionm[i_pionm1];
		RecMdcKalTrack::setPidType  (RecMdcKalTrack::pion);
	...
	for(int i_pionm2 = i_pionm1 + 1; i_pionm2 < iPionm.size(); i_pionm2++)
	{
		...
		RecMdcKalTrack::setPidType  (RecMdcKalTrack::pion);
		HepLorentzVector p4_pionm2 = ((*itTrk)->mdcKalTrack())->p4(PDG::Pion);
