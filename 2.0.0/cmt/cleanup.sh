...
${CMTROOT}/mgr/cmt cleanup -sh -pack=SYJ_Lambda_c -version=2.0.0 -path=/workfs/bes/suyj/BES3_WorkArea/7.0.3  $* >${cmtSYJ_Lambda_ctempfile}
if test $? != 0 ; then
  echo >&2 "${CMTROOT}/mgr/cmt cleanup -sh -pack=SYJ_Lambda_c -version=2.0.0 -path=/workfs/bes/suyj/BES3_WorkArea/7.0.3  $* >${cmtSYJ_Lambda_ctempfile}"
  cmtcleanupstatus=2
