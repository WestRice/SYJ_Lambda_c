...
StatusCode Sigma0PionEta::execute(){
	...
	for(int iLmd = 0; iLmd < iLmdpp.size(); ++iLmd)
		for( int iShr = 0; iShr < iGam.size(); ++iShr)
		{
			...
			HepLorentzVector p4_sigma0, p4_sigma01c;
			if(isGoodSigma0(&wtpLmd1s[iLmd], gamShr, sigma0_mass, p4_sigma0, sigma0_chis, p4_sigma01c))
			{
	...
	for(int iALmd = 0; iALmd < iLmdpm.size(); ++iALmd)
		for( int iShr = 0; iShr < iGam.size(); ++iShr)
		{
			...
			HepLorentzVector p4_Asigma0, p4_Asigma01c;
			if(isGoodSigma0(&wtpALmd1s[iALmd], gamShr, Asigma0_mass, p4_Asigma0, Asigma0_chis, p4_Asigma01c))
			{
...
bool Sigma0PionEta::isGoodSigma0(WTrackParameter *lmd,RecEmcShower *gammaShr,double& sigma0_mass,HepLorentzVector& p4_sigma0,double& sigma0_chis,HepLorentzVector& p4_sigma0_1c){
		...
		if(sigma0_chis>2500) return false;
